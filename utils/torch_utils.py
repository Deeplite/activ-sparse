...
def get_output_shapes(model, input_shape, modules, device):
    shapes = {}
    def get_activation(name):
        def hook(module, i, o):
            shapes[name] = o.detach().shape
        return hook
    handles = []
    for name, module in modules.items():
        handles.append(module.register_forward_hook(get_activation(name)))
    out = model(torch.randn(input_shape).to(device))
    for handle in handles:
        handle.remove()
    return shapes

def skip_layers(modules_toprune, toskip):
    modules_toskip = {}
    for name, module in modules_toprune.items():
        if ('conv1x1' in toskip and module.kernel_size == (1, 1)) or \
           ('conv3x3' in toskip and module.kernel_size == (3, 3)) or \
           ('conv7x7' in toskip and module.kernel_size == (7, 7)):
            modules_toskip[name] = module
    for name in modules_toskip:
        modules_toprune.pop(name)
    return modules_toprune, modules_toskip
